...
import { readMultiProfile, tunnelIdentityFor } from "./profile.js";
import { assertProfileName } from "./mounts.js";

export function assertSavedMultiProfile(name: string) {
...
export function multiTunnelChoicePayload(name: string, zoneHint?: string): Record<string, unknown> {
  const profile = assertSavedMultiProfile(name);
...
export async function setupMultiNamedTunnel(input: {
  name: string;
  zone?: string;
  hostname?: string;
...
> {
  const profile = assertSavedMultiProfile(input.name);
  const workspaceId = tunnelIdentityFor(profile.name);
  ...
  if (!result.ok) {
    return {
      ...
      payload: {
        ...multiTunnelChoicePayload(profile.name, zone),
        ok: false,
        fallback: false,
        implicitQuick: false,
